...
if [[ -n "$SOURCE_OVERRIDE" ]]; then
  ...
  step "Building sm"
  # --manifest-path: build this checkout, not whichever one the caller is in.
  cargo build --release --bin sm --manifest-path "$REPO_ROOT/Cargo.toml" --target-dir "$SM_TARGET_DIR" \
