...
impl SessionStore {
    ...
    fn recover_session_runtime_launch(&self, launch_id: &str) -> Result<()> {
        ...
        let codex_fork_restore_needs_checked_teardown = launch.provider == "codex-fork"
            && launch.is_authorized_restore_intent()
            && state
                .get("sessions")
        ...
        if codex_fork_restore_needs_checked_teardown {
            match session_runtime.teardown_session_for_restore(&launch.tmux_session) {
                ...
                RestoreTmuxTeardownOutcome::Inconclusive { reason } => {
                    let failure_reason = format!(
                        "failed to confirm prior runtime teardown during restore recovery: {reason}"
                    );
                    mark_runtime_launch_teardown_inconclusive(
                        &mut state,
                        launch_id,
                        &launch.session_id,
                        &failure_reason,
    ...
    pub fn retire_core_session(
        ...
        let authority = requester_session_id
            .map(RetireAuthority::authenticated_parent)
            .unwrap_or_else(|| RetireAuthority::server_lifecycle("internal_session_store"));
        self.retire_core_session_authorized(session_id, authority, None)
    }
    ...
    pub fn retire_core_session_authorized(
        ...
        let now = now_rfc3339();
        session.insert("status".to_owned(), Value::String("stopped".to_owned()));
        mark_session_retired(session, &now, authority.terminal_provenance(&now, None));
    ...
    pub fn retire_core_session_with_runtime(
        ...
        let authority = requester_session_id
            .map(RetireAuthority::authenticated_parent)
            .unwrap_or_else(|| RetireAuthority::server_lifecycle("internal_session_store"));
        self.retire_core_session_with_runtime_authorized(session_id, authority, None, runtime)
    }
    ...
    pub fn retire_core_session_with_runtime_authorized(
        ...
        let intent = authority.terminal_provenance(&requested_at, None);
        {
            let sessions = ensure_sessions_array_mut(&mut state)?;
            let session = session_object_mut(sessions, session_id).ok_or_else(|| {
                anyhow::anyhow!("session {session_id} disappeared before runtime retirement")
            })?;
            session.insert(
                "retirement_intent".to_owned(),
                serde_json::to_value(&intent)?,
            );
        }
        // The authenticated intent survives a crash between the request and the
        // tmux operation. Startup deliberately does not act on it; it is audit
        // evidence, not permission to infer or repeat a teardown.
        self.write_raw_json_value(&state)?;
        let tmux_was_killed = match session_runtime.kill_session(&tmux_session) {
            Ok(result) => result,
            Err(error) => {
                let sessions = ensure_sessions_array_mut(&mut state)?;
                if let Some(session) = session_object_mut(sessions, session_id) {
        ...
        if let Some(next_status) = codex_fork_status_for_event(event).filter(|next_status| {
            ...
            if next_status == "stopped" {
                ...
                set_terminal_provenance_if_absent(
                    session,
                    TerminalProvenance::provider_natural_exit(&now, "codex_fork_lifecycle_event"),
                );
...
impl RetireAuthority {
    ...
    pub fn authenticated_parent(session_id: impl Into<String>) -> Self {
        Self::AuthenticatedParent {
            session_id: session_id.into(),
        }
    }

    fn server_lifecycle(source: impl Into<String>) -> Self {
    ...
    fn can_retire(&self, target: &Map<String, Value>) -> bool {
        match self {
            Self::AuthenticatedParent { session_id } => {
    ...
    fn rejection_outcome(&self, target: &Map<String, Value>) -> CoreRetireOutcome {
        match self {
            Self::AuthenticatedParent { .. }
                if json_text(target.get("parent_session_id")).is_none() =>
            {
    ...
    fn terminal_provenance(
        ...
        match self {
            Self::Operator { source } => TerminalProvenance::explicit_retire(
                requested_at,
                None,
                "operator",
                source,
            ...
            Self::AuthenticatedParent { session_id } => TerminalProvenance::explicit_retire(
                requested_at,
                Some(session_id),
            ...
            Self::ServerLifecycle { source } => TerminalProvenance::explicit_retire(
                requested_at,
                None,
                "server_lifecycle",
                source,
...
fn retire_result(session_id: &str) -> CoreRetireResult {
    CoreRetireResult {
        ok: true,
        session_id: session_id.to_owned(),
...
fn deliver_runtime_text_to_session_with_ready_fence_raw(
    ...
    if terminal_delivery_failure {
        ...
        set_terminal_provenance_if_absent(
            session,
            TerminalProvenance::tmux_disappearance(&now, "runtime_delivery_failed"),
        );
    }
    Ok((status, delivered))
}
...
fn deliver_urgent_runtime_text_to_session_raw(
    ...
    if terminal_delivery_failure {
        ...
        set_terminal_provenance_if_absent(
            session,
            TerminalProvenance::tmux_disappearance(&now, "runtime_urgent_delivery_failed"),
        );
    }
    Ok((status, delivered))
}
...
fn mark_session_runtime_missing_terminal(state: &mut Value, session_id: &str) -> Result<()> {
    ...
    if !raw_session_is_stopped(session) {
        ...
        set_terminal_provenance_if_absent(
            session,
            TerminalProvenance::tmux_disappearance(&now, "runtime_reconciliation_missing"),
        );
    }
    Ok(())
}
...
/// Persist an inconclusive pre-launch teardown without claiming absence.
///
/// The old runtime may still be live, so its durable credential is deliberately
/// left untouched and the seat remains operator-visible and fenced in `error`.
fn mark_runtime_launch_teardown_inconclusive(
    state: &mut Value,
    launch_id: &str,
    session_id: &str,
    failure_reason: &str,
    ...
    if let Some(session) = session_object_mut(sessions, session_id) {
        ...
        session.insert(
            "error_message".to_owned(),
            Value::String(format!("runtime_teardown_inconclusive: {failure_reason}")),
        );
    }
    Ok(())
}
...
pub struct SessionRecord {
    ...
    #[serde(default)]
    pub stopped_at: Option<String>,
    /// Durable attribution for the terminal transition. Older records remain
    /// explicitly unknown rather than being inferred from a later restart.
    #[serde(default)]
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn codex_fork_restore_inconclusive_prelaunch_teardown_preserves_old_credential() {
        let root = unique_temp_path("codex-fork-restore-prelaunch-inconclusive");
...
"#,
        ...
        let old_credential = "old-runtime-secret";
        let old_credential_sha256 = sha256_text(old_credential);
        ...
        assert_eq!(
            ...
            sha256_text(old_credential)
        );
        assert!(session["error_message"]
            .as_str()
            .unwrap()
            .contains("runtime_teardown_inconclusive"));
        let launch = &state["session_runtime_launches"][0];
        assert_eq!(launch["status"], "failed");
        assert_ne!(launch["credential_sha256"], sha256_text(old_credential));
    ...
    #[cfg(unix)]
    #[test]
    fn codex_fork_restore_recovery_checks_teardown_before_rotating_credential() {
        let root = unique_temp_path("codex-fork-restore-recovery-prelaunch-inconclusive");
        fs::create_dir_all(&root).unwrap();
        let tmux = root.join("tmux");
        let tmux_log = root.join("tmux.log");
        fs::write(
...
"#,
        ...
        let proposed_credential_sha256 = sha256_text("proposed-runtime-secret");
        let state_file = root.join("sessions.json");
        fs::write(
            &state_file,
            json!({
                "sessions": [{
                    ...
                    "session_credential_sha256": old_credential_sha256,
                    "working_dir": root,
                    "tmux_session": "codex-fork-restore03",
                    "log_file": root.join("restore.log"),
                    "status": "stopped",
                    "completion_status": "killed",
                    "stopped_at": "2026-08-18T00:00:01Z",
                ...
                "session_runtime_launches": [{
                    ...
                    "provider_resume_id": "durable-root",
                    "restore_authorized": true,
                    "credential_sha256": proposed_credential_sha256,
                    "status": "launching",
        ...
        assert_eq!(
            session["session_credential_sha256"],
            sha256_text("old-runtime-secret")
        );
        assert_eq!(launch["credential_sha256"], proposed_credential_sha256);
        assert_eq!(launch["status"], "failed");
        assert!(launch["failure_reason"]
            .as_str()
            .unwrap()
        ...
        assert_eq!(
            fs::read_to_string(&tmux_log).unwrap(),
            "kill-session -t =codex-fork-restore03\n"
        );
        let _ = fs::remove_dir_all(root);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn codex_fork_restore_rotates_credential_only_after_confirmed_teardown() {
        let root = unique_temp_path("codex-fork-restore-post-teardown-credential");
        fs::create_dir_all(&root).unwrap();
        let tmux = root.join("tmux");
        let tmux_log = root.join("tmux.log");
        fs::write(
    ...
    fn approved_single_reparent_retargets_parent_derived_routing_across_stores() {
        ...
        assert!(matches!(
            store.retire_core_session_authorized(
                "child001",
                RetireAuthority::authenticated_parent("oldpar01"),
                Some(old_credential),
            ),
        ...
        assert!(matches!(
            store.retire_core_session_authorized(
                "child001",
                RetireAuthority::authenticated_parent("newpar01"),
                Some(new_credential),
            ),
