...
fn workload_metrics(
    ...
    Ok(json!({
        ...
        "jobs_failed_24h": jobs.iter().filter(|job| matches!(job.state.as_str(), "failed" | "timed_out" | "memory_exceeded" | "process_limit_exceeded") && in_last_day(job.finished_at.as_deref(), now)).count(),
