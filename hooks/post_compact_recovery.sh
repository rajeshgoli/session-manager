...
#
# Runs once the compacted session is back up. Two jobs, both answered by a single
# request:
...
#
# The handoff path comes back on the acknowledgement's own response rather than
# from a follow-up read of /sessions/{id}. That route is an ordinary API
# endpoint behind Google auth, which a hook on a remote node cannot satisfy — it
# carries a node hook secret and nothing else, and only /hooks/* accepts that.
#
# Only the reinjection JSON may reach stdout: Claude parses this hook's stdout.
...
BODY=$(jq -c -n --arg sid "$CLAUDE_SESSION_MANAGER_ID" \
  '{session_id: $sid, event: "compaction_complete"}' 2>/dev/null) || exit 0

RESPONSE=$(curl -s --max-time 3 --connect-timeout 2 -X POST "$HOOK_URL" \
  "${HEADERS[@]}" -d "$BODY" 2>/dev/null) || exit 0

HANDOFF_PATH=$(printf '%s' "$RESPONSE" | jq -r '.last_handoff_path // empty' 2>/dev/null) || exit 0
