...
def _build_local_auth_overrides(env_values: dict[str, str]) -> dict:
    ...
    ssh_proxy_command = env_values.get("SSH_PROXY_COMMAND", "").strip()
    ...
    if ssh_proxy_command:
        external_access["ssh_proxy_command"] = ssh_proxy_command
    ...
    async def _cleanup_stale_telegram_topics_once(self) -> dict[str, int]:
        ...
        for record in list(self.session_manager.telegram_topic_registry.values()):
            ...
            if session and session.telegram_thread_id != record.thread_id:
                active_topic = session.telegram_thread_id
                stale_reason = (
                    f"session no longer points to this topic (active={active_topic})"
                    if active_topic
                    else "session retired this topic"
                )
