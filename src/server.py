...
def create_app(
    ...
    def _termux_attach_metadata(session: Session, descriptor: Optional[dict[str, Any]]) -> dict[str, Any]:
        ...
        local_attach_script = (
            "attach_log=\"${TMPDIR:-/tmp}/sm-attach-$$-${RANDOM:-0}.log\"; "
            ": >\"$attach_log\" 2>/dev/null || attach_log=\"/tmp/sm-attach-$$.log\"; "
            "attach_cleanup() { "
            "stty sane 2>/dev/null || true; "
            "rm -f \"$attach_log\" 2>/dev/null || true; "
            "}; "
            "show_attach_error() { "
            "if [ -s \"$attach_log\" ]; then "
            "cat \"$attach_log\" >/dev/tty 2>/dev/null || cat \"$attach_log\" >&2; "
            "fi; "
            "}; "
            "run_attach() { "
  ...
  <body>
    ...
    async def update_session(
        ...
        if friendly_name is not None or is_em is not None:
            app.state.session_manager._save_state()
            renamer = getattr(app.state.session_manager, "queue_provider_native_rename", None)
