...
def mock_session_manager():
    ...
    mock.queue_provider_native_rename = AsyncMock(return_value=True)
...
class TestUpdateSession:
    ...
    def test_update_friendly_name(self, test_client, mock_session_manager, sample_session):
        ...
        mock_session_manager.queue_provider_native_rename.assert_awaited_once_with(sample_session, "new-name")
    ...
    def test_update_friendly_name_times_out_slow_telegram_rename(
        self,
        mock_session_manager,
        mock_output_monitor,
        caplog,
    ):
        """PATCH /sessions/{id} bounds slow Telegram rename latency."""
        import logging
        from src.server import create_app
        from fastapi.testclient import TestClient
        ...
        mock_notifier = MagicMock()
        mock_notifier.rename_session_topic = AsyncMock(side_effect=slow_rename)
