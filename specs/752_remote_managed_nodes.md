...
monitor, attach to, and tear down agent sessions that physically run on a **second machine**.

Mental model:

- **Studio** — the always-on host. Runs the one SM server, the one Telegram bot, the one
  state store. This is the default node; nothing changes for sessions placed here.
- **MacBook** — a registered remote node. When a session is explicitly placed on it, the agent
  process and its tmux live on the MacBook's hardware, but the Studio's SM still owns it: it
  appears in `sm all`, reports status, is attachable, and is reachable from Telegram exactly
  like a local session.

One brain, multiple execution hosts. Default mode is "everything runs on the Studio"; placing a
session on the MacBook is an explicit, per-session choice.
...
- **Hook callbacks are the critical blocker.** Agents report status via
  ...
  (`notify_server.sh:6`). On a remote node this default hits the *MacBook's* loopback, not the
  Studio. SM injects `CLAUDE_SESSION_MANAGER_ID` into the session shell via send-keys
...
- **Transcript read assumes local FS.** The hook handler `POST /hooks/claude`
  ...
  message / native title. For a remote session that path is on the MacBook.
- **Status polling + log locality.** `output_monitor.py` polls `has-session` and exit
  ...
  (`tmux_controller.py:980`). For a remote session both the log and the pane live on the MacBook.
...
There is no way for the single SM to run a session on a different physical machine while still
owning it. The only way to use the MacBook's hardware today is a second, independent SM instance
with its own tmux, state, and (conflicting) Telegram bot — which is invisible to the Studio SM
and violates the single-brain model.
...
6. Default behavior is **unchanged**: omitting a node places the session on the local (Studio)
   host. Zero behavior change for existing single-host users.
...
- Sharing a single working tree across machines. Code syncs via git; each node has its own
  checkout (paths align because both use `~/projects`).
- More than two nodes in practice, though the design must not hard-code a count.
...
```bash
sm new ~/projects/foo                 # runs on the local (Studio) host
...
```bash
sm new ~/projects/foo --node macbook  # agent runs on the MacBook
sm dispatch trainer --repo ~/projects/ml --node macbook --provider codex
```
...
a4af4272  engineer1  claude    studio    working
7c1f90ab  trainer    codex     macbook   waiting-input
```

Attach is node-transparent:

```bash
sm attach 7c1f90ab    # SM routes the attach to the MacBook's tmux
```
...
single `NodeRunner` abstraction that every tmux invocation flows through. For the local node it
...
command construction almost verbatim, leans on SSH for transport + auth (already in use via the
configured cloudflared SSH proxy), and needs no new long-running service to build, deploy, and
version-lock against SM. ControlMaster neutralizes the latency objection that would otherwise
favor a daemon.
...
3. **Per-session node, threaded through spawn.** `node` flows from the API → `create_session` →
   `_create_session_common` → `create_session_with_command`, defaulting to `"local"`.
4. **Hook callback fix (critical).** When spawning on a remote node, the managed-shell export
   step (`tmux_controller.py:536`) also exports `SM_HOOK_URL` pointing at the Studio's
   SM-reachable address (from `nodes.<id>.hook_url`, e.g. the LAN address or cloudflared origin).
   The remote agent then POSTs hooks straight to the Studio.
5. **Transcript locality fix.** For remote sessions, `notify_server.sh` extracts the last
   ...
   inlined fields when present and only falls back to local `transcript_path` reads for local
   nodes. (Backward compatible: local sessions keep current behavior.)
...
```python
node: str = "local"          # node id; "local" = the host SM runs on
```

- `models.py`: add field (near `:297`), serialize in `to_dict` (`:394`), parse in `from_dict`
  (`:492`, defaulting missing → `"local"` for backward compat with existing `sessions.json`).
...
nodes:
  default: local
  registry:
    local: {}                                  # the host SM runs on; no SSH
    macbook:
      ssh: "rajesh@macbook.local"              # or a cloudflared proxy host
      ssh_proxy_command: "cloudflared access ssh --hostname %h"   # optional
      control_path: "~/.ssh/cm-sm-macbook.sock"
      hook_url: "http://studio.local:8420/hooks/claude"  # Studio addr the node can reach
      projects_root: "~/projects"              # for path validation/sanity
```

## API & CLI Changes

- `CreateSessionRequest` (`server.py`) gains optional `node: str = "local"`; validated against the
  registry (unknown node → 400).
...
- Remote control is plain SSH; rely on key auth + the existing cloudflared SSH proxy. No new
  inbound port on the MacBook beyond sshd.
- `SM_HOOK_URL` for a remote node points at the Studio over a trusted path (LAN or authenticated
...
- **Custom node-agent daemon on the MacBook** (HTTP/WS service the Studio calls to spawn/send/
  capture, streaming events back). Architecturally clean — solves log locality by streaming and
  lets hooks POST to localhost-then-forward. Rejected for Phase 1: it's a substantial new service
  (auth, lifecycle, deploy, version lock-step with SM) that mostly re-implements what SSH +
  ControlMaster already provide. Revisit if SSH transport proves limiting (e.g. high-frequency
  control, or many nodes).
- **Bind the SM server to the LAN and run a 2nd SM on the MacBook** — rejected; that's two brains,
  not one, and reintroduces the Telegram 409 problem.

## Implementation Plan (maps to sub-tickets)

1. **Node model + config registry + CLI surfacing** — add `Session.node`, `nodes:` config,
   `--node` flag, `sm all`/TUI column, `sm nodes`. Local-only still; no remote execution yet.
   (De-risks the data/UX layer with zero behavior change.)
...
6. **Node liveness + failure UX** — `node-unreachable` status, reconnay/backoff, docs.

## Tests

- Unit: `NodeRunner` argv construction (local vs ssh), registry validation, `Session.node`
...
- Regression: full existing suite with `node="local"` — zero behavior change.
- Liveness: node down → sessions show `node-unreachable`; node returns → recovers.

## Edge Cases

- Legacy `sessions.json` without `node` → treated as `local`.
- Node unreachable mid-session: tmux keeps running on the MacBook; SM shows `node-unreachable`
...
- Clock/path skew: validate `working_dir` exists on the target node before spawn; clear error if
  not.
- Provider specifics: codex/codex-fork binary resolution must run on the **target** node, not the
  Studio (`_create_session_common` currently resolves locally at `:2693-2716`).

## Acceptance Criteria

- `sm new --node macbook` runs the agent on the MacBook; `sm all` shows `node=macbook`.
...
attach, and liveness. Proposed sub-tickets follow the six-step Implementation Plan above; each is
independently shippable and ordered so step 1 lands behavior-neutral. File sub-tickets referencing
