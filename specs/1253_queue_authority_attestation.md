...
4. Query `SMJobCopyDictionary(kSMDomainUserLaunchd, "com.rajeshgoli.session-manager-rust")` in-process. Its numeric `PID` must equal the kernel peer PID. `XPC_SERVICE_NAME` and response fields are not launchd authority.
5. Resolve that PID with `proc_pidpath`. It must equal `/Users/rajesh/projects/session-manager/.local/bin/sm-server` exactly.
6. Read that live PID's code-sign identity with `csops(pid, CS_OPS_IDENTITY)`. It must equal `com.rajeshgoli.sm-server`.
7. Send the bounded request. Reject timeout, EOF before newline, oversized output, multiple frames, non-UTF-8, or malformed JSON.
8. Require response schema `sm.queue_authority.response.v1`.
9. Require `service.pid` to equal the kernel peer PID. Require the exact launchd label, executable path, and code-sign identifier above.
10. Only then validate and consume the queue job fields. All missing or malformed fields fail closed.
