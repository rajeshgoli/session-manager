...
| message queue DB | Must preserve retained messages, scheduled reminders, remind registrations, parent wakes, notify-on-stop delivery, and Codex review request registrations. Job watches remain archive/rollback-only. | Copy DB; run reminder recovery and queue-depth checks for retained delivery; record any deprecated rows in the migration ledger. | Restore DB atomically if Rust claims/updates messages or reminders incorrectly. No mixed ownership. |
