...
| `watch-job` and external job watches | Do not port. Retained state is empty. Start supervised commands with `sm queue run --notify <session> -- <command>`; queue jobs recover durably and automatically send an `[sm queue]` completion wake. Processes started outside the queue cannot be registered later. |
